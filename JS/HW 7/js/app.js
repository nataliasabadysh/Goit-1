...
const getAllNames = users => users.map(e => e.name);
console.group("ДЗ 1");
...
//
// Почу проверку не проходит
console.assert(getAllNames(users) === hw1, "дз1");
console.groupEnd();

// №2
const getUsersByEyeColor = (users, color) =>
  users.filter(e => e.eyeColor === color);
console.group("ДЗ 2");
...
const getUsersByGender = (users, gender) =>
  users.filter(e => e.gender === gender).map(e => e.name);

console.group("ДЗ 3");
console.log(getUsersByGender(users, "male")); // [ 'Moore Hensley', 'Ross Vazquez', 'Carey Barr', 'Blackburn Dotson' ]
console.groupEnd();

const getInactiveUsers = users => users.filter(e => e.isActive === false);

console.group("ДЗ 4");
console.log(getInactiveUsers(users)); // [объект Moore Hensley, объект Ross Vazquez, объект Blackburn Dotson]
console.groupEnd();

const getUserByEmail = (users, email) => users.find(e => e.email === email);
...
const getUsersWithAge = (users, min, max) =>
  users.filter(e => e.age >= min && e.age <= max);
...
const getUsersByFriend = (users, name) => {
 return users.filter(e => e.friends.includes(name)).map(e => e.name)
};
